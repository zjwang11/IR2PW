...
for i in range(27*ntot):
    ...
    for j in range(count):
        if (abs(newsphr[r][0]-hr[j][0])+abs(newsphr[r][1]-hr[j][1])+abs(newsphr[r][2]-hr[j][2]) < eps):
            for k in range(9):
                for l in range(nat*nat):
                    ii=l+nat*nat*(k+9*i)
                    for kk in range(9):
                        if (abs(newsphr[l+k*nat*nat][3]-hr[j+count*kk][3])+abs(newsphr[l+k*nat*nat][4]-hr[j+count*kk][4]) < eps):
