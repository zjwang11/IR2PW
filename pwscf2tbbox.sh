...
crstd=(1.000 0.000 0.000 -0.500000 0.866025 0.000 0.000 0.000 1.000)
x=0
for ((i=0;i<"$sym";i++))
do
nx[i]="${nxyz[x]}"
...
if [[ ${typex[i]} == cart. ]]; then
 ...
 # zs=`echo "(${nx[i]}*${crystax[2]}+${ny[i]}*${crystax[5]}+${nz[i]}*${crystax[8]})"|bc`
  xs=`echo "(${nx[i]}*${crstd[0]}+${ny[i]}*${crstd[3]}+${nz[i]}*${crstd[6]})"|bc`
  ys=`echo "(${nx[i]}*${crstd[1]}+${ny[i]}*${crstd[4]}+${nz[i]}*${crstd[7]})"|bc`
  zs=`echo "(${nx[i]}*${crstd[2]}+${ny[i]}*${crstd[5]}+${nz[i]}*${crstd[8]})"|bc`
  nf=`echo "sqrt(($xs)^2+($ys)^2+($zs)^2)"|bc`
  n_x[i]=`echo "scale=6;$xs/$nf"|bc`
  n_y[i]=`echo "scale=6;$ys/$nf"|bc`
  n_z[i]=`echo "scale=6;$zs/$nf"|bc`
fi
  if [[ ${alpha[i]} -gt -180 && ${alpha[i]} -lt 0 ]];then
    n_x[i]=`echo "${n_x[i]}*(-1)"|bc`
    n_y[i]=`echo "${n_y[i]}*(-1)"|bc`
    n_z[i]=`echo "${n_z[i]}*(-1)"|bc`
  fi
  ii=`echo "$i+1"|bc`
  printf "%5s%12.6f%12.6f%12.6f%12.6f%12.6f%12.6f%12.6f%12.6f\n" $ii ${deta[i]} ${alpha[i]} ${n_x[i]} ${n_y[i]} ${n_z[i]} ${tau1[i]} ${tau2[i]} ${tau3[i]}  >> $name
done
